import { updateProductSchema } from "@/lib/domains/catalog/catalog.validators";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    const admin = createSupabaseAdminClient();
    const { data, error } = await admin.from("products").update(parsed.data).eq("id", id).select("*").maybeSingle();
